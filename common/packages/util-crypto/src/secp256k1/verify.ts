...
import { assert, u8aEq, u8aToU8a } from '@tetcoin/util';
...
export function secp256k1Verify (message: Uint8Array | string, signature: Uint8Array | string, address: Uint8Array | string, hashType: HashType = 'blake2'): boolean {
  const isVapory = hashType === 'keccak';
  ...
  const signingAddress = secp256k1Hasher(hashType, isVapory ? secp256k1Expand(publicKey) : publicKey);
  const inputAddress = u8aToU8a(address);

  // for Vapory (keccak) the last 20 bytes is the address
  return isVapory
